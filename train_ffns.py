...
def train_process_fsdp(local_rank, chunked_layers_params, seeds, batch_size, model_size):
    ...
    def gather_layer_params(l, prev_sharded_ps=None, prev_handles=None):
        def init_all_gather(sharded_p):
        ...
        def finalize_all_gather(sharded_ps, handles):
            for h in handles:
                h.wait()
        ...
        if prev_sharded_ps is not None:
            ...
            layer_params=finalize_all_gather(prev_sharded_ps, prev_handles)
        else: # For the first fn call, don't call GatherAll
            assert prev_handles is None
            layer_params=None
        
        sharded_ps_lst, handles = [], []
        if l>=0 and l<len(chunked_layers_params): # Only start new async GatherAll if it's valid layer
            for i in range(2):
                sharded_p, handle = init_all_gather(chunked_layers_params[l][i])
    ...
    for x, dloss_dx in mock_data(seeds, batch_size, model_size):
        ...
        y=x
        acts = []
        _, sharded_ps, handles = gather_layer_params(0)
                
        for l in range(layers):
            acts.append(y)
            layer_params, sharded_ps, handles  = gather_layer_params(l+1, sharded_ps, handles)
            y = tlayer_ffn_fwd(layer_params, y)
            
        # Backward + optimizer (in-place SGD)
        # initiate async GatherAll for the penultime layer (the last layer's params were already gathered in the forward)
        _, sharded_ps, handles = gather_layer_params(layers-2) 
        batch_dloss_dx = dloss_dx
        for i in reversed(range(layers)):
            if i<layers-1: # For the last layer, we don't need to recall GatherAll
                layer_params, sharded_ps, handles = gather_layer_params(i-1, sharded_ps, handles)
